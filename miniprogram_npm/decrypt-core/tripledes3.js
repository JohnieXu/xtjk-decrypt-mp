...
var CryptoJS = CryptoJS || function (u, l) {
  var d = {},
    ...
    concat: function concat(a) {
      ...
      a = a.sigBytes;
      this.clamp();
      if (f % 4) for (var t = 0; t < a; t++) {
        c[f + t >>> 2] |= (m[t >>> 2] >>> 24 - 8 * (t % 4) & 255) << 24 - 8 * ((f + t) % 4);
      } else if (65535 < m.length) for (t = 0; t < a; t += 4) {
        c[f + t >>> 2] = m[t >>> 2];
...
(function () {
  ...
  u.enc.Base64 = {
    stringify: function stringify(d) {
      ...
      if (n = s.charAt(64)) for (; d.length % 4;) {
        d.push(n);
      }
      return d.join("");
    },
...
(function (u) {
  ...
  q = q.MD5 = v.extend({
    ...
    _doProcessBlock: function _doProcessBlock(r, e) {
      for (var a = 0; 16 > a; a++) {
        var c = e + a,
            m = r[c];
...
CryptoJS.lib.Cipher || function (u) {
  var l = CryptoJS,
      ...
      v = d.Cipher = s.extend({
    cfg: n.extend(),
    createEncryptor: function createEncryptor(m, a) {
      return this.create(this._ENC_XFORM_MODE, m, a);
    },
    createDecryptor: function createDecryptor(m, a) {
      return this.create(this._DEC_XFORM_MODE, m, a);
    },
    init: function init(m, a, b) {
    ...
    _ENC_XFORM_MODE: 1,
    _DEC_XFORM_MODE: 2,
    _createHelper: function _createHelper(m) {
      return {
        encrypt: function encrypt(f, b, e) {
          return ("string" == typeof b ? c : a).encrypt(m, f, b, e);
        },
        decrypt: function decrypt(f, b, e) {
  ...
  var b = l.mode = {},
    ...
    createEncryptor: function createEncryptor(a, f) {
      return this.Encryptor.create(a, f);
    },
    createDecryptor: function createDecryptor(a, f) {
  ...
  r.Encryptor = r.extend({
    processBlock: function processBlock(a, f) {
      var b = this._cipher,
          c = b.blockSize;
      x.call(this, a, f, c);
      b.encryptBlock(a, f);
  ...
  var e = d.CipherParams = n.extend({
    ...
    decrypt: function decrypt(a, c, b, e) {
      e = this.cfg.extend(e);
    ...
    encrypt: function encrypt(c, b, e, d) {
      d = this.cfg.extend(d);
    ...
    decrypt: function decrypt(c, b, e, d) {
      d = this.cfg.extend(d);
...
(function () {
  ...
  var d = CryptoJS,
    ...
    encryptBlock: function encryptBlock(b, a) {
      this._doCryptBlock(b, a, this._subKeys);
    },
    decryptBlock: function decryptBlock(b, a) {
      this._doCryptBlock(b, a, this._invSubKeys);
    },
    _doCryptBlock: function _doCryptBlock(e, a, c) {
      this._lBlock = e[a];
      this._rBlock = e[a + 1];
...
function createContent(config, appkey) {
  // 最终生成密文
  // 生成随机数
  var randData = s2as(32); // console.log("随机数:"+randData);
  ...
  var src = strToHexCharCode(JSON.stringify(config)); // console.log('16进制明文:'+src);
...
function resolveContent(actionInfo, appkey) {
  ...
  var randData = actionInfo.substring(0, 32); // 获取应用密文

  var singData = actionInfo.substring(32, actionInfo.length); // 获取过程密钥

  var processKey = encryptByDES(mpiKey, randData, true); // 解密singData

  var actionInfoString = decryptByDES(processKey, singData); // console.log('actionInfoString:'+actionInfoString);

  var json = JSON.parse(actionInfoString); // console.log('parse json:'+json);
...
function encryptByDES(mpiKey, message, isSessionKey) {
  mpiKey = mpiKey + mpiKey.substring(0, 16); //console.log("秘钥："+mpiKey);
  //转16进制
  // console.log(message);
  //过程密钥不做填充

  if (!isSessionKey) {
    message = padding80(message); // console.log('padding:'+message);
  }

  message = CryptoJS.enc.Hex.parse(message); // console.log(`mpiKey: ${mpiKey}`)
...
function decryptByDES(mpiKey, ciphertext) {
  ...
  var keyHex = CryptoJS.enc.Hex.parse(mpiKey);
  ciphertext = CryptoJS.enc.Hex.parse(ciphertext); // direct decrypt ciphertext
  ...
  data = hexCharCodeToStr(data); // console.log('data2:'+data);
...
module.exports.padding80 = padding80;
...
module.exports.encryptByDES = encryptByDES;
module.exports.decryptByDES = decryptByDES;
